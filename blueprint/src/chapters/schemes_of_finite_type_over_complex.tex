...
\end{proposition}
\begin{proof}
	Done in {\sf mathlib4} by Andrew Yang.
\end{proof}
...
Let $P$ be a local property of ring homorphisms and $\Phi : \schemeOf{X} \to \schemeOf{Y}$ be an open immersion between schemes, then affine locally $P$ holds for $\Phi$.
\end{proposition}
\begin{proof}
Done in {\sf mathlib4} by Andrew Yang.	
\end{proof}
...
\begin{proof}
  ...
  Alternatively: use \Cref{thm:affine-locally-open-immersion}.
\end{proof}
...
\begin{proof}
Use \Cref{thm:affine-locally-source-cover}.
\end{proof}
